...
func (h *DocumentHandlers) DownloadDocumentHandler() http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		if !auth.HasRole(r.Context(), "ADMIN") {
			...
			if callerDept == "" || callerDept != strconv.FormatInt(int64(doc.DepartmentID), 10) {
				httputil.Error(w, http.StatusForbidden, "forbidden: only your department documents are allowed")
				return
			}
		}
		if doc.IsHidden {
...
func ListDocumentsByFolderHandler(q *db.Queries) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		if !auth.HasRole(r.Context(), "ADMIN") {
			filtered := make([]db.ListDocumentsByFolderRow, 0, len(docs))
...
func MoveDocumentHandler(q *db.Queries) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		if err != nil {
			httputil.Error(w, http.StatusBadRequest, "target folder not found")
			return
		}
		...
		if !isAdmin {
			callerDept, _ := auth.DepartmentIDFromContext(r.Context())
			callerDeptNum, parseErr := strconv.ParseInt(callerDept, 10, 32)
			if parseErr != nil || int32(callerDeptNum) != doc.DepartmentID || int32(callerDeptNum) != folder.DepartmentID {
				httputil.Error(w, http.StatusForbidden, "forbidden: you can move only your department documents")
				return
			}
...
func (h *DocumentHandlers) HideDocumentHandler() http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		if isAdmin {
			...
			callerDept, _ := auth.DepartmentIDFromContext(ctx)
			if callerDept == "" || callerDept != strconv.FormatInt(int64(doc.DepartmentID), 10) {
				httputil.Error(w, http.StatusForbidden, "forbidden: only your department documents are allowed")
				return
			}
	...
	return func(w http.ResponseWriter, r *http.Request) {
		...
		if !auth.HasRole(r.Context(), "ADMIN") {
			...
			if callerDept == "" || callerDept != strconv.FormatInt(int64(deptID), 10) {
				httputil.Error(w, http.StatusForbidden, "forbidden: only your department search is allowed")
				return
			}
		}
