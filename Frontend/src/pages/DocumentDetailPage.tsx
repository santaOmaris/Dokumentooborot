...
export default function DocumentDetailPage() {
  ...
  const [visaApproverId, setVisaApproverId] = useState<number | null>(null)
  ...
  const visaMutation = useMutation({
    mutationFn: () => orchestratorApi.sendForVisa(docId, {
      note: visaNote.trim(),
      approver_id: visaApproverId ?? undefined,
    }),
    onSuccess: () => {
      message.success('Отправлен на визирование')
      setVisaOpen(false)
      setVisaNote('')
      setVisaApproverId(null)
      invalidateStatus()
    },
  ...
  const currentUserDeptID = Number(me?.department_id ?? 0)
  const docDepartmentID = Number(doc?.department_id ?? me?.department_id ?? 0)
  const canRequestApproval = (isHead || isAdmin) && ['DRAFT', 'PENDING_VISA'].includes(workflowStatus)
  const canApproveOrReject =
    isHead &&
    currentUserDeptID !== 0 &&
    docDepartmentID !== 0 &&
    currentUserDeptID === docDepartmentID &&
    ['PENDING_VISA', 'PENDING_BOSS'].includes(workflowStatus)
  const visaManagers = (users ?? []).filter((u) => {
    if (!u.is_head) return false
    if (!docDepartmentID) return true
    return Number(u.department_id) === docDepartmentID
  })
  ...
  return (
    <div>
      ...
      <Space wrap style={{ marginBottom: 16 }}>
        {status === 'DRAFT' && (
          <Button type="primary" icon={<SendOutlined />}
            onClick={() => {
              setVisaApproverId(visaManagers[0]?.id ?? null)
              setVisaOpen(true)
            }}>
            На визирование
          </Button>
        ...
        <Button
          ...
          title={!canRequestApproval ? `Недоступно в статусе ${workflowStatus}` : undefined}
        >
      ...
      <Modal title="Комментарий к визированию" open={visaOpen}
        onCancel={() => { setVisaOpen(false); setVisaNote(''); setVisaApproverId(null) }}
        onOk={() => visaMutation.mutate()} confirmLoading={visaMutation.isPending}
        okText="Отправить"
        okButtonProps={{ disabled: !visaNote.trim() || !visaApproverId }}>
        <Select
          showSearch
          placeholder="Выберите начальника"
          style={{ width: '100%', marginBottom: 12 }}
          value={visaApproverId}
          onChange={setVisaApproverId}
          options={visaManagers.map((u) => ({ value: u.id, label: `${u.full_name} (${u.login})` }))}
          notFoundContent="В отделе не найдено начальников"
        />
        <TextArea
          rows={4}
          value={visaNote}
