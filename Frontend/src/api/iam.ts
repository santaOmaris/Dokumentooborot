...
function normalizeDepartmentId(value: unknown): number | null {
  ...
  const maybe = value as { Int32?: number; Valid?: boolean }
...
function normalizeCurrentUser(raw: unknown): CurrentUser {
  ...
  return {
    ...
    login: String(u.login ?? ''),
    system_role: String(u.system_role ?? 'USER').toUpperCase() as CurrentUser['system_role'],
    is_head: normalizeBoolean(u.is_head),
...
export const iamApi = {
  ...
  listDepartments: () => api.get<Department[]>('/api/iam/departments').then(r => r.data),
